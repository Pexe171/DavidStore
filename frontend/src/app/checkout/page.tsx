  ...
  const handleSubmit = async (
    ...
    try {
      ...
      setStatus({
        type: 'success',
        message:
          'Pedido confirmado! Você receberá atualizações no e-mail informado.'
      })
      setForm(initialForm)
      setTimeout(() => {
        router.push('/')
      ...
      setStatus({
        type: 'error',
        message:
          'Não foi possível concluir o pedido. Tente novamente mais tarde.'
      })
    } finally {
      setLoading(false)
    }
    ...
    return (
      <section className="container" style={{ padding: '2rem 0 4rem' }}>
        <div className="card" style={{ textAlign: 'center' }}>
          ...
          <p>
            Adicione itens ao carrinho para seguir com o checkout humanizado da
            David Store.
          </p>
        </div>
      </section>
      ...
      <div className="card" style={{ display: 'grid', gap: '2rem' }}>
        <header>
          ...
          <p style={{ color: '#64748b' }}>
            Atendimento consultivo, confirmação instantânea e personalização
            inspirada no varejo omnichannel.
          </p>
        </header>
        <form onSubmit={handleSubmit} style={{ display: 'grid', gap: '1rem' }}>
          ...
          >
            ...
            <button type="submit" className="btn-primary" disabled={loading}>
              {loading
                ? 'Confirmando pedido...'
                : 'Confirmar compra com David Pay'}
            </button>
          </div>
        </form>
        {status.message && (
